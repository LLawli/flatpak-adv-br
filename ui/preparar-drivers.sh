...
if [ -n "$REMOTEID_TESTE" ]; then
    ...
    # interruptor só.
    #
    # O /tmp deste sandbox É compartilhado entre as instâncias do aplicativo,
    # ao contrário do que estava escrito aqui antes; medido. Mas ele mora no
    # diretório de execução e SOME NO LOGOUT: quem gravar ali perde a conta de
    # teste na próxima sessão. Por isso o caminho que o RemoteID espera vira um
    # link para os dados do aplicativo, que persistem.
    #
    # Quando o RemoteID chega primeiro e cria o diretório de verdade, o estado
    # é resgatado para cá — a não ser que já exista outra conta gravada, e aí a
    # escolha é de quem usa, não nossa.
    if [ -d /tmp/remoteid-teste ] && [ ! -L /tmp/remoteid-teste ]; then
        if [ -e "$REMOTEID/teste/state.json" ] &&
            [ -e /tmp/remoteid-teste/state.json ]; then
            echo "adv-br: há conta de teste em /tmp/remoteid-teste e em $REMOTEID/teste." >&2
            echo "     Apague a que não interessa; a de /tmp some no logout." >&2
        elif cp -a /tmp/remoteid-teste/. "$REMOTEID/teste"/ 2>/dev/null &&
            rm -rf /tmp/remoteid-teste &&
            ln -sfn "$REMOTEID/teste" /tmp/remoteid-teste 2>/dev/null; then
            echo "adv-br: a conta de teste estava em /tmp, que some no logout; trouxe para $REMOTEID/teste." >&2
        else
            echo "adv-br: não consegui trazer /tmp/remoteid-teste para os dados do aplicativo; o modo de teste vai gravar num lugar que não persiste." >&2
        fi
    else
